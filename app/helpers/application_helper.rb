module ApplicationHelper
  ...
  def boot_elements
    {
      ...
      reset_btn: %w[btn btn-sm bg-gray-d border-light-gray rounded-left-border border-right-0 text-sm font-weight-light text-info],
      search_input: %w[form-control bg-gray-d border-light-gray rounded-right-border border-left-0]
    }
  end

  def table_strip(i)
    i.even? ? 'bg-soft-a-stripe' : 'bg-soft-b-stripe'
  end
  ...
  def abbrv_tagline(item)
    item.hattr('csv_tags', 'invoice_tagline')
  end
end
