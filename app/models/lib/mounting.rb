class Mounting
  ...
  class StandardMounting < Mounting
    def self.builder
      select_menu(field_name, field_kind, [Framing.builder, Border.builder, Matting.builder], tags)
    end
  end

  class CanvasMounting < Mounting
    def self.builder
      select_menu(field_name, field_kind, [Framing.builder, Matting.builder], tags)
    end
  end
  ...
  class Framing < Mounting
    def self.builder
      select_field = select_field(field_name, field_kind, Option.builder(['framed', 'custom framed'], field_kind))
      field_set(field_name, field_kind, Dimension::FieldGroup.options(Dimension::FieldGroup.width_height, field_kind).prepend(select_field), tags)
    end
  end
  ...
  class SculptureMounting < Mounting
    def self.builder
      field_set(field_name, field_kind, self.subclasses.map{|klass| klass.builder}, tags)
    end
    ...
    class Case < SculptureMounting
      def self.builder
        ...
        field_set(field_name, field_kind, [radio_button(field_name, field_kind), select_menu(build_name([field_name, 'dimensions']), field_kind, options)], tags)
      end
    end
