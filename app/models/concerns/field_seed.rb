...
module FieldSeed
  ...
  class_methods do
    ...
    def config_target(c, f_hsh={})
    	f_hsh[:attrs] = c.build_attrs
      f_hsh[:tags] = c.build_parent_tags(f_hsh[:attrs])
      f_hsh[:targets] = c.build_targets(f_hsh[:attrs][:kind])
    ...
    def build_targets(kind)
      if target_tags = asc_detect(:target_tags, :respond_to?)
        targets.map{|target| {type: 'Option', kind: kind, field_name: target, tags: asc_select_merge(:target_tags, target).reject{|k,v| v.blank?}}}
      elsif targets
    ...
    def add_field(f_class, kind, f_name, f_hsh)
    ...
    def add_and_assoc_targets(f, targets)
    	targets.map{|ff_hsh| add_field(to_class(ff_hsh[:type]), ff_hsh[:kind], ff_hsh[:field_name], ff_hsh)}.map{|ff| f.assoc_unless_included(ff)} if targets
    end
