  ...
  class_methods do
    ...
    def product_fields(store)
      ...
      products.each_with_object([]) do |p,set|
        ...
        #puts "tag_keys: #{tag_keys}"
        tags = product_tags(p, tag_keys)

        #puts "#{product_name}: tags: #{tags}"
    ...
    #     #products << assign_set_or_group(assoc_hsh, p_hsh[:assocs], p_hsh)
    ...
    def combine_fields(origin, assoc_hsh)
      origin.each_with_object([]) do |p_hsh,products|
        p_hsh = assign_set_or_group(assoc_hsh, p_hsh[:assocs], p_hsh)
        if p_hsh[:group].any?
          [p_hsh[:f]].product(*p_hsh[:group]).map{|a| a + p_hsh[:set]}.map{|p| products.append(p)}
        else
    ...
    #     set.append({set: [store.dig(*vals[0..-2])], group:[], assocs: vals[-1]})
    #   end
    # end
    ...
    def field_group(m, store)
      attrs_tags_and_assocs(m).each_with_object(store) do |f, store|
    ...
    def build_assocs(*assoc_meths)
      assoc_meths.each_with_object({}) do |m, h|
        h.merge!({m => asc_select(m).map{|c| c.public_send(m)}.flatten})
        #h.merge!({m => asc_select(m).each_with_object([]){|c,assoc| c.public_send(m) + assoc}})
      end
    end
